...
class WebRTCCamera extends HTMLElement {
    ...
    ws.onmessage = (ev) => {
      if (typeof ev.data === "string") {
        ...
        if (!video.paused && video.seekable.length) {
          ...
          if (video.seekable.end(0) - video.currentTime > 1) {
  ...
  renderCustomGUI(card) {
    ...
    if (
      ...
      this.config.shortcuts.services &&
      this.config.shortcuts.services.length > 0
    ) {
  ...
  async renderGUI(hass) {
    ...
    if (
      ...
      const orientation =
        config.orientation && config.orientation in map
          ? map[config.orientation]
          : "left";
      style.textContent +=
        `
                .shortcuts-` +
        this.getUniqueShortcutsKey() +
        ` {
                    position: absolute;
                    top: calc(12px + ` +
        (config.top ? this.prepareMargin(config.top) : "0px") +
        `);
                    left: calc(12px + ` +
        (config.left ? this.prepareMargin(config.left) : "0px") +
        `);
                }
                .shortcuts-` +
        this.getUniqueShortcutsKey() +
  ...
  disconnectedCallback() {
    console.log("aaa", "disconnectedCallback");
  ...
  prepareMargin(margin) {
    return !isNaN(parseFloat(margin)) && isFinite(margin)
      ? margin + "px"
      : margin;
  }
}
...
window.customCards.push({
  ...
  preview: false,
  description:
    "WebRTC Camera allows you to watch RTSP-camera stream without any delay",
});
